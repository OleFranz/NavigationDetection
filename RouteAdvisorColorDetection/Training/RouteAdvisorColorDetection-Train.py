    ...
    def load_data(files=None, type=None):
        ...
        for file in os.listdir(DATA_PATH):
            if file in files:
                ...
                img = np.array(img)
                UpperRed = np.array([255, 110, 110])
                LowerRed = np.array([160, 0, 0])
                UpperGreen = np.array([150, 255, 230])
                LowerGreen = np.array([0, 200, 0])
                RedMask = cv2.inRange(img, LowerRed, UpperRed)
                GreenMask = cv2.inRange(img, LowerGreen, UpperGreen)
                cv2.circle(RedMask, (round(img.shape[0] * 0.142), round(img.shape[0] * 0.146)), round(img.shape[0] * 0.077), (0, 0, 0), round(img.shape[0] * 0.025))
                cv2.circle(GreenMask, (round(img.shape[0] * 0.142), round(img.shape[0] * 0.146)), round(img.shape[0] * 0.077), (0, 0, 0), round(img.shape[0] * 0.025))
                NewRedColor = random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)
                NewGreenColor = random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)
                img[RedMask != 0] = NewRedColor
                img[GreenMask != 0] = NewGreenColor
                label = NewRedColor + NewGreenColor
                label = [i / 255 for i in label]
...
class ConvolutionalNeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(IMG_WIDTH * IMG_HEIGHT * COLOR_CHANNELS, 1024)
    ...
    def forward(self, x):
        x = x.view(-1, self.IMG_WIDTH * self.IMG_HEIGHT * self.COLOR_CHANNELS)
        x = self.fc1(x)
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.dropout(x)
...
def main():
    ...
    if not os.path.exists(f"{PATH}/Training/TensorBoard-Logs"):
        os.makedirs(f"{PATH}/Training/TensorBoard-Logs")

    # Delete previous tensorboard logs
    for obj in os.listdir(f"{PATH}/Training/TensorBoard-Logs"):
        try:
            shutil.rmtree(f"{PATH}/Training/TensorBoard-Logs/{obj}")
        except:
            os.remove(f"{PATH}/Training/TensorBoard-Logs/{obj}")

    # Tensorboard setup
    summary_writer = SummaryWriter(f"{PATH}/Training/TensorBoard-Logs", comment="RouteAdvisorColorDetection-Training", flush_secs=20)
    ...
    for i in range(5):
        try:
            ...
            torch.jit.save(last_model, os.path.join(MODEL_PATH, f"RouteAdvisorColorDetectionModel-LAST-{TRAINING_DATE}.pt"), _extra_files=metadata)
    ...
    for i in range(5):
        try:
            ...
            torch.jit.save(best_model, os.path.join(MODEL_PATH, f"RouteAdvisorColorDetectionModel-BEST-{TRAINING_DATE}.pt"), _extra_files=metadata)
